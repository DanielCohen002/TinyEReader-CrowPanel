...
- Reading progress indicator, one of four: off (the default), a thin bottom bar, percentage, or page fraction. Percent/Fraction are tacked onto the end of the last line of text rather than their own row — the line trims back a whole word at a time to make genuine room when needed, rather than drawing over whatever was already there — so none of the four costs a line of reading space
...
- **Progress** -- cycles None / Bar / Percent / Fraction, off (None) by default. One at a time, not layered -- Bar shows the thin bottom bar instead of corner text, None shows neither. Fraction shows "current page / total pages" (a real pagination sweep, not a byte-offset estimate) -- see [Reading progress](#reading-progress) below. Switching into Fraction, or changing Text size while Fraction is active, re-sweeps the book and briefly shows "Loading..." while it does.
